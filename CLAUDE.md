...
## 5.4 Autonomous Fast-Start Triggers (Repo-Local Override)
- **Local to this repo only — deliberately excluded from `templates/CLAUDE.md.template` so downstream `hydrate init` users receive a clean baseline, and kept outside the managed block markers above so `hydrate sync`/`hydrate init` never overwrite it.** This overrides the generic "### 5.4 Autonomous Fast-Start Triggers" inside the managed block.
